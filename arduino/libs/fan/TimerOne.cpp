 ...
 *  Modified June 2009 by Michael Polli and Jesse Tane to fix a bug in setPeriod() which caused the
 *timer to stop
 ...
 *  - Add (long) assignments and casts to TimerOne::read() to ensure calculations involving tmp,
 *ICR1 and TCNT1 aren't truncated
 ...
 *  - Disable interrupts whilst TCTN1 == 0.  Datasheet vague on this, but experiment shows that
 *overflow interrupt
 *    flag gets set whilst TCNT1 == 0, resulting in a phantom interrupt.  Could just set to 1, but
 *gets inaccurate
 ...
 *  - renamed startBottom() to start(). This breaks some old code that expects start to continue
...
void TimerOne::setPeriod(long microseconds)  // AR modified for atomic access
{
  long cycles = (F_CPU / 2000000) * microseconds;  // the counter runs backwards after TOP,
                                                   // interrupt is at BOTTOM so divide microseconds
                                                   // by 2
...
void TimerOne::pwm(char pin, int duty, long microseconds)  // expects duty cycle to be 10 bit (1024)
{
  ...
  setPwmDuty(pin, duty);
  resume();  // Lex - make sure the clock is running.  We don't want to restart the count, in case
             // we are starting the second WGM
...
{                               // rember! phase and freq correct mode counts up to then down again
  ...
  tmp =
      ((tcnt1 > tmp) ? (tmp) : (long)(ICR1 - tcnt1) +
                                   (long)ICR1);  // AR amended to add casts and reuse previous TCNT1
